...
create index if not exists garmin_activities_user_idx
  on public.garmin_activities (garmin_user_id);

create index if not exists garmin_activities_start_time_idx
  on public.garmin_activities (start_time desc);


-- ===============================
-- GARMIN ACTIVITY DETAILS (samples-based)
-- ===============================
...
create table if not exists public.garmin_activity_details (
  ...
  -- store compressed/lightweight data only
  ...
  -- raw detail WITHOUT huge samples
  raw_json jsonb not null,
...
create index if not exists garmin_activity_details_user_idx
  on public.garmin_activity_details (garmin_user_id);